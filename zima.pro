function zIma, timeout=timeout, file=file, settingsfile=settingsfile

  if n_elements(file) eq 0 then file = getenv('IDL_TMPDIR')+'ima.txt'
  if file_test(file, /regular) eq 1 then file_delete, file
  if n_elements(settingsfile) eq 0 then begin
    settingsfile=''
    flag=0
  endif else begin
   if file_test(settingsfile) ne 1 then message, 'Settingsfile not existing '+settingsfile  
    flag=1
  endelse
  cmd = string(format='(%"GetTextFile, \"%s\", Ima, \"%s\", %d")',file,settingsfile,flag)
