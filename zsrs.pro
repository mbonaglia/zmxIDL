...
; SRS: Surface sag
;
function zSrs, timeout=timeout, file=file, settingsfile=settingsfile

  if n_elements(file) eq 0 then file = getenv('IDL_TMPDIR')+'srs.txt'
  ...
  if n_elements(settingsfile) eq 0 then begin
   ...
   flag=1
  endelse
  cmd = string(format='(%"GetTextFile, \"%s\", Srs, \"%s\", %d")',file,settingsfile,flag)
  ...
  res = read_ascii(file, count=count, data_start=12)
