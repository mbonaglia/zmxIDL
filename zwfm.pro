...
function zWfm, timeout=timeout, file=file, settingsfile=settingsfile

  if n_elements(file) eq 0 then file = getenv('IDL_TMPDIR')+'wfm.txt'
  ...
  if n_elements(settingsfile) eq 0 then begin
   ...
   if file_test(settingsfile) eq 0 then message, 'Settingsfile not existing '+settingsfile  
    flag=1
  endelse
  cmd = string(format='(%"GetTextFile, \"%s\", Wfm, \"%s\", %d")',file,settingsfile,flag)
